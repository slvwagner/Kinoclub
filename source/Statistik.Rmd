...
```{r}
mean_median <- df_temp|>
  reframe(`Durchschnittlicher Gewinn pro Vorführung [CHF]` = mean(`Gewinn/Verlust Filmvorführungen [CHF]`)|>round5Rappen()|>round(2),
          `Median Gewinn pro Vorführung [CHF]` = median(`Gewinn/Verlust Filmvorführungen [CHF]`)|>round5Rappen()|>round(2)
          )
...
if(df_temp$`Gewinn/Verlust Tickets [CHF]`|>mean() > df_temp$`Gewinn/Verlust Tickets [CHF]`|>median()){
  gewinn <- df_temp$`Gewinn/Verlust Tickets [CHF]`|>median()
}else{
  gewinn <- df_temp$`Gewinn/Verlust Tickets [CHF]`|>mean()
}
...
```{r echo=FALSE}
df_temp|>
  reframe(`Abgabe and die SUISA [CHF]` = sum(`SUISA-Vorabzug [CHF]`),
          `Abgaben and den Verleiher [CHF]` = sum(`MWST [CHF]`),
          `MWST [CHF]` = sum(`MWST [CHF]`)
          )|>
  knitr::kable()
...
if(df_temp$`Verleiherabzug [CHF]`|>mean() > df_temp$`Verleiherabzug [CHF]`|>median()){
  gewinn <- df_temp$`Verleiherabzug [CHF]`|>mean()
}else{
  gewinn <- df_temp$`Verleiherabzug [CHF]`|>median()
}
