...
df_temp <- df_Abrechnung|>
  ...
  select(Datum, Filmtitel, `SUISA-Vorabzug [CHF]`)
...
df_temp <- df_Abrechnung|>
  arrange(Datum)|>
  mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))|>
  select(Datum,Filmtitel, `Verleiherabzug [CHF]`)
...
```{r}
df_temp|>
  reframe(`Summe [CHF]` = sum(`Verleiherabzug [CHF]`)|>round5Rappen())|>
  knitr::kable()
```
...
```{r}
df_temp <- df_Abrechnung|>
  select(Datum, Filmtitel, `Suisa Nummer`, `MWST [CHF]`)|>
  arrange(Datum)|>
  mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))
df_temp|>
  knitr::kable()
...
```{r}
df_temp|>
  reframe(`Summe [CHF]` = sum(`MWST [CHF]`))|>
  knitr::kable()
```
...
```{r}
df_temp <- df_Abrechnung|>
  reframe(`SUISA-Vorabzug [CHF]`= -sum(`SUISA-Vorabzug [CHF]`, na.rm = T),
          `Sonstige Kosten [CHF]` = -sum(`Sonstige Kosten [CHF]`, na.rm = T),
          `Verleiherabzug [CHF]` = -sum(`Verleiherabzug [CHF]`, na.rm = T),
...
Filmvorführungen <- df_Abrechnung|>
  select(Datum, Filmtitel, `Gewinn/Verlust Tickets [CHF]`)|>
  reframe(`Summe [CHF]` = sum(`Gewinn/Verlust Tickets [CHF]`, na.rm = T))
