...
df_temp <- atelierkino_abo|>
  ...
  rename(url = id)|>
  mutate(
    creation = paste0(day(creation),".", month(creation), ".", year(creation)),
    first_use = paste0(day(first_use),".", month(first_use), ".", year(first_use)),
    last_use = paste0(day(last_use),".", month(last_use), ".", year(last_use)),
    expiration = paste0(day(expiration),".", month(expiration), ".", year(expiration))
    )|>
  mutate(url = paste0('<a href="', url, '" target="_blank">', url, '</a>'),
         `Noch nicht eingelöster Betrag [CHF]` = (6 - count_use)*13)|>
  mutate(`Anzahl Tickets` = 6-count_use)
```
...
df_temp <- atelierkino_gutschein|>
  ...
  mutate(id = paste0("https://www.advance-ticket.ch/badge/", id))|>
  rename(url = id)|>
  mutate(creation = paste0(day(creation),".", month(creation), ".", year(creation)),
       first_use = paste0(day(first_use),".", month(first_use), ".", year(first_use)),
       last_use = paste0(day(last_use),".", month(last_use), ".", year(last_use)),
       expiration = paste0(day(expiration),".", month(expiration), ".", year(expiration))
       )|>
  ...
  select(-first_use, -last_use, -count_use)
```
