...
if(nrow(df_temp) > 0) {
  ...
  df_temp <-df_Verleiher_Rechnnung |>
    slice(ii)|>
    mutate(`Link Datum` = ymd(`Link Datum`))
  df_temp
  
  df_temp|>
    select(Verleiher, Betrag)|>
    knitr::kable(digits = 2)
...
df_temp|>
  ...
  mutate(`Verleiher-Abzug [CHF]` = `Verleiher-Abzug [CHF]` * c_verteilprodukt[ii])|>
  knitr::kable(digits = 2)
...
df_GV_Eintritt|>
  filter(Datum %in% c(c_Datum, c_linkDatum) & `Suisa Nummer` == c_suisa)|>
  select(`Sonstige Kosten [CHF]`)|>
  mutate(`Reklamematerial und Porto [CHF]` = `Sonstige Kosten [CHF]` * c_verteilprodukt[ii],
...
df_GV_Eintritt|>
  ...
  mutate(`MWST auf die Verleiherrechnung [CHF]` = `MWST auf die Verleiherrechnung [CHF]` * c_verteilprodukt[ii])|>
  knitr::kable(digits = 2)
```
...
Eventeinnahmen <- Eventeinnahmen|>
  reframe(`Summe [CHF]` = sum(`Betrag [CHF]`,na.rm = T) * c_verteilprodukt[ii])
