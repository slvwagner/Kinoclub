...
```{r echo=FALSE}
df_temp <- df_Abrechnung|>
  filter(Datum %in% (c_Datum) & `Suisa Nummer` == c_suisa)|>
...
if(is.na(df_temp$`Verleiherrechnungsbetrag [CHF]`)){ # keine Verleiherrechnung vorhanden
  if(df_temp$`Kinoförderer gratis?`){
      ...
      df_temp1 <- tibble(Bezeichnung = c("Umsatz","Suisa-Vorabzug (Kinofördere abgerechnet)","Verleiherabzug (Kinofördere abgerechnet)", "MWST"),
                     Verteilprodukt = df_temp$Verteilprodukt, 
                     `Betrag [CHF]` = c(
                       c_Umsatz,
                       -(df_temp$`SUISA-Vorabzug [CHF]`) ,
  ...
  if(df_temp$`Kinoförderer gratis?`){
    df_temp1 <- tibble(Bezeichnung = c("Umsatz","Suisa-Vorabzug","Verleiherrechnung (inkl. MWST)"),
        ...
        df_temp1 <- tibble(Bezeichnung = c("Umsatz","Suisa-Vorabzug (Kinofördere abgerechnet)","Verleiherrechnung (inkl. MWST)"),
                       Verteilprodukt = df_temp$Verteilprodukt, 
                       `Betrag [CHF]` = c(
                         c_Umsatz,
                         -(df_temp$`SUISA-Vorabzug [CHF]`) ,
