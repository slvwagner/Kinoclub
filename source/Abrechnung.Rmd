...
if(nrow(df_temp) > 0) {
  ...
  df_temp <-df_Abrechnung |>
    ...
    mutate(Firmennamen = Firmennamen[!is.na(Firmennamen)])|>
    filter(Datum == c_Date[ii])
  df_temp
  ...
  df_temp|>
    select(Firmennamen, Verleiherrechnungsbetrag)|>
    knitr::kable()
}
```
...
df_temp|>
  ...
  mutate(`Verleiher-Abzug [CHF]` = `Verleiher-Abzug [CHF]` * c_verteilprodukt)|>
  knitr::kable(digits = 2)
...
df_GV_Eintritt|>
  filter(Datum %in% c(c_Datum) & `Suisa Nummer` == c_suisa)|>
...
df_GV_Eintritt|>
  ...
  mutate(`MWST auf die Verleiherrechnung [CHF]` = `MWST auf die Verleiherrechnung [CHF]` * c_verteilprodukt)|>
  knitr::kable(digits = 2)
```
...
Eventeinnahmen <- Einnahmen_und_Ausgaben$Einnahmen|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[1], 
         Datum %in% c(c_Date[ii], c_linkDatum))|>
  select(Bezeichnung, Betrag)|>
  mutate(`Betrag [CHF]` = Betrag * c_verteilprodukt[ii],
         Betrag = NULL)
...
Eventeinnahmen <- Eventeinnahmen|>
  reframe(`Summe [CHF]` = sum(`Betrag [CHF]`,na.rm = T) * c_verteilprodukt)
...
Eventausgaben <- Einnahmen_und_Ausgaben$Ausgaben|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[1], 
         Spieldatum %in% c(c_Date[ii], c_linkDatum))|>
  select(Bezeichnung, Betrag)|>
  mutate(`Betrag [CHF]` = Betrag * c_verteilprodukt,
         Betrag = NULL)
